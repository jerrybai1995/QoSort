...
func SampleSort(A []doublepair) {
    sample_sort(A, 0, len(A))
}

func sample_sort(A []doublepair, i, j int) {
    n := j - i
    if n < QUICKSORT_THRESHOLD {
        Qsort_parallel(pairs(A), i, j, 0)
        return
    }
    ...
    sample_set := make([]doublepair, sample_set_size)
    ...
    for b := 0; b < num_blocks; b++ {
        ...
        go func() {
            ...
            Qsort_parallel(pairs(sketch), offset, offset + size, QSORT_POOL_SIZE)
    ...
    for b := 0; b < num_buckets; b++ {
        ...
        go func() {
            ...
            Qsort_parallel(pairs(A), istart, iend, QSORT_POOL_SIZE)
            wg2.Done()
        }()
    }
    wg2.Wait()
 ...
 */
func merge_seq(A []doublepair, A_offset int, A_size int,
               pivots []doublepair, num_pivots int,
