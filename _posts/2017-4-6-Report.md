...
Generally, our attention has focused on three areas:
  ...
  * A quick, parallel quicksort in Go with multiple optimizations based on cache locality and load balancing. 
  * A quick, parallel samplesort in Go that is cache friendly (still in progress for some final tuning, etc.)
...
- **Further optimized worker parallel quicksort by split-by-3 initially**: 3.94 seconds

<div class="imgcap">
<img src="/QoSort/assets/images/bar.png" style="width: 70%">
<div class="thecap" style="color: white; font-size: 14pt">Figure: Comparing various quicksorts that we built</div>
</div>
...
**The best baseline model we found that supports generic type was only able to sort 100M float64-pairs in about 7.3 seconds (a starred open source project)**, which is larger than our best result!
...
With our current version of SampleSort, we achieved 0.8s with sorting 10M float64-pair elements on a 20-core machine (40 logical processors). We are carrying out more tests and performance tweaks along the way.